...
<head>
    <meta charset="UTF-8">
    <title>Suppliers Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Suppliers & Vendors Directory Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Suppliers Report</h4>
            ...
            <div class="col-12">
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        <div class="col-4">
                            Total Suppliers: {{ number_format($totalSuppliers) }}
                        </div>
                        <div class="col-4 border-start border-end">
                            Active Suppliers: {{ number_format($activeSuppliers) }}
                        </div>
                        <div class="col-4">
                            Inactive Suppliers: {{ number_format($inactiveSuppliers) }}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <h6 class="fw-bold text-dark mb-2">Registered Suppliers List</h6>
            ...
            <tbody>
                @forelse($suppliers as $index => $supplier)
                    <tr>
                        ...
                        <td class="fw-semibold">{{ $supplier->company_name }}</td>
                        <td class="font-monospace">{{ $supplier->gst_number ?? 'N/A' }}</td>
                        <td>{{ $supplier->email ?? 'N/A' }}</td>
                        <td>{{ $supplier->phone ?? 'N/A' }}</td>
                        <td>{{ $supplier->address ?? 'N/A' }}</td>
                        @if(auth()->user()->isSuperAdmin())
                            <td>{{ $supplier->firm->name ?? 'N/A' }}</td>
                        @endif
                        ...
                        <td class="text-center font-monospace">{{ $supplier->created_at ? $supplier->created_at->format('d-m-Y') : 'N/A' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ auth()->user()->isSuperAdmin() ? 9 : 8 }}" class="text-center py-3 text-muted">No suppliers found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
