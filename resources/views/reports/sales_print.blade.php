...
<head>
    <meta charset="UTF-8">
    <title>Sales Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Sales Transactions & Revenue Summary Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Sales Report</h4>
            ...
            <div class="col-12">
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        <div class="col-3">Total Orders: {{ number_format($totalOrdersCount) }}</div>
                        <div class="col-3 border-start border-end">Grand Total: ₹{{ number_format($totalGrandTotal, 2) }}</div>
                        <div class="col-3 border-end">Total Received: ₹{{ number_format($totalPaidAmount, 2) }}</div>
                        <div class="col-3">Pending Due: ₹{{ number_format($totalPendingAmount, 2) }}</div>
                    </div>
                </div>
            </div>
        </div>

        <h6 class="fw-bold text-dark mb-2">Sales Orders List</h6>
            ...
            <tbody>
                @forelse($sales as $index => $sale)
                    <tr>
                        ...
                        <td class="font-monospace fw-semibold">{{ $sale->invoice_number }}</td>
                        <td class="text-center font-monospace">{{ $sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('d-m-Y') : 'N/A' }}</td>
                        <td>{{ $sale->customer->company_name ?? 'N/A' }}</td>
                        <td>{{ $sale->project_name ?? 'N/A' }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($sale->subtotal, 2) }}</td>
                        <td class="text-end font-monospace">+₹{{ number_format($sale->tax_amount, 2) }}</td>
                        <td class="text-end font-monospace">-₹{{ number_format($sale->discount_amount ?? 0, 2) }}</td>
                        <td class="text-end font-monospace">+₹{{ number_format($sale->shipping_cost ?? 0, 2) }}</td>
                        <td class="text-end fw-bold font-monospace">₹{{ number_format($sale->grand_total, 2) }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($sale->paid_amount, 2) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="11" class="text-center py-3 text-muted">No sales records found.</td>
                    </tr>
                @endforelse
            </tbody>
            @if(count($sales) > 0)
                <tfoot class="table-light border-top font-outfit fw-bold">
                    <tr>
                        ...
                        <td class="text-end font-monospace">₹{{ number_format($totalPaidAmount, 2) }}</td>
                    </tr>
                </tfoot>
            @endif
        </table>
