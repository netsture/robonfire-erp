...
<head>
    <meta charset="UTF-8">
    <title>Purchases Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Purchases Procurement & Orders Summary Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Purchases Report</h4>
            ...
            <div class="col-12">
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        ...
                        <div class="col-3 border-end">Total Paid: ₹{{ number_format($totalPaidAmount, 2) }}</div>
        ...
        <h6 class="fw-bold text-dark mb-2">Purchase Orders List</h6>
            ...
            <tbody>
                @forelse($purchases as $index => $purchase)
                    <tr>
                        ...
                        <td class="text-center font-monospace">{{ $purchase->purchase_date ? \Carbon\Carbon::parse($purchase->purchase_date)->format('d-m-Y') : 'N/A' }}</td>
                        <td>{{ $purchase->supplier->company_name ?? 'N/A' }}</td>
                        <td>{{ $purchase->project_name ?? 'N/A' }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($purchase->subtotal, 2) }}</td>
                        <td class="text-end font-monospace">+₹{{ number_format($purchase->tax_amount, 2) }}</td>
                        <td class="text-end font-monospace">-₹{{ number_format($purchase->discount_amount ?? 0, 2) }}</td>
                        <td class="text-end font-monospace">+₹{{ number_format($purchase->shipping_cost ?? 0, 2) }}</td>
                        <td class="text-end fw-bold font-monospace">₹{{ number_format($purchase->grand_total, 2) }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($purchase->paid_amount, 2) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="11" class="text-center py-3 text-muted">No purchase records found.</td>
                    </tr>
                @endforelse
            </tbody>
