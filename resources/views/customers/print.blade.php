...
<head>
    <meta charset="UTF-8">
    <title>Customers Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Customer & Client Directory Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Customers Report</h4>
                ...
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        ...
                        <div class="col-4 border-start border-end">
                            Active Customers: {{ number_format($activeCustomers) }}
                        </div>
                        <div class="col-4">
                            Inactive Customers: {{ number_format($inactiveCustomers) }}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <h6 class="fw-bold text-dark mb-2">Registered Customers List</h6>
            ...
            <tbody>
                @forelse($customers as $index => $customer)
                    <tr>
                        ...
                        <td class="font-monospace">{{ $customer->gst_number ?? 'N/A' }}</td>
                        <td>{{ $customer->email ?? 'N/A' }}</td>
                        <td>{{ $customer->phone ?? 'N/A' }}</td>
                        <td>{{ $customer->address ?? 'N/A' }}</td>
                        @if(auth()->user()->isSuperAdmin())
                            <td>{{ $customer->firm->name ?? 'N/A' }}</td>
                        @endif
                        ...
                        <td class="text-center font-monospace">{{ $customer->created_at ? $customer->created_at->format('d-m-Y') : 'N/A' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ auth()->user()->isSuperAdmin() ? 9 : 8 }}" class="text-center py-3 text-muted">No customers found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
