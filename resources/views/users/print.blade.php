...
<head>
    <meta charset="UTF-8">
    <title>Users Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">User Accounts & Access Management Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Users Report</h4>
            ...
            <div class="col-12">
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        ...
                        <div class="col-4">
                            Inactive Users: {{ number_format($inactiveUsers) }}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <h6 class="fw-bold text-dark mb-2">User Accounts List</h6>
            ...
            <tbody>
                @forelse($users as $index => $user)
                    <tr>
                        ...
                        <td>{{ $user->firm->name ?? 'Global / Superadmin' }}</td>
                        <td>{{ $user->email }}</td>
                        <td>{{ $user->phone ?? 'N/A' }}</td>
                        <td>{{ ucfirst($user->role ?? ($user->roles->first()->name ?? 'User')) }}</td>
                        ...
                        <td class="text-center font-monospace">{{ $user->created_at ? $user->created_at->format('d-m-Y') : 'N/A' }}</td>
                    </tr>
                @empty
                    <tr>
