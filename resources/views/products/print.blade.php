...
<head>
    <meta charset="UTF-8">
    <title>Products Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Products & Inventory Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Products Report</h4>
                ...
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        ...
                        <div class="col-4 border-start border-end">
                            Total Stock Qty: {{ number_format($totalStock) }}
                        </div>
                        <div class="col-4">
        ...
        <h6 class="fw-bold text-dark mb-2">Master Product List</h6>
            ...
            <tbody>
                @forelse($products as $index => $product)
                    <tr>
                        ...
                        <td class="font-monospace">#{{ $product->product_identifier }}</td>
                        ...
                        <td class="text-end fw-bold">₹{{ number_format($product->selling_price, 2) }}</td>
                        <td class="text-center font-monospace">
                            <span class="{{ $product->stock_quantity <= $product->alert_quantity ? 'text-danger fw-bold' : '' }}">
                                {{ number_format($product->stock_quantity) }} {{ $product->unit }}
                            </span>
                        </td>
                        @if(auth()->user()->isSuperAdmin())
                            <td>{{ $product->firm->name ?? 'N/A' }}</td>
                        @endif
                        ...
                        <td class="text-center font-monospace">{{ $product->created_at ? $product->created_at->format('d-m-Y') : 'N/A' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ auth()->user()->isSuperAdmin() ? 12 : 11 }}" class="text-center py-3 text-muted">No products found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
