...
<head>
    <meta charset="UTF-8">
    <title>Project Expense Management Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Project Expense Management Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Project Expenses Report</h4>
        ...
        <h6 class="fw-bold text-dark mb-2">Project Expense Details List</h6>
            ...
            <tbody>
                @forelse($expenses as $index => $expense)
                    <tr>
                        ...
                        <td class="text-center font-monospace">{{ $expense->expense_date ? \Carbon\Carbon::parse($expense->expense_date)->format('d-m-Y') : 'N/A' }}</td>
                        <td class="fw-semibold">{{ $expense->project->project_name ?? 'N/A' }}</td>
                        <td class="font-monospace">{{ $expense->project->po_number ?? 'N/A' }}</td>
                        <td class="fw-semibold">{{ $expense->item_name }}</td>
                        <td>{{ $expense->description ?? 'N/A' }}</td>
                        <td class="text-end fw-bold font-monospace">₹{{ number_format($expense->amount, 2) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-3 text-muted">No expenses found.</td>
                    </tr>
                @endforelse
            </tbody>
            @if(count($expenses) > 0)
                <tfoot class="table-light border-top font-outfit fw-bold">
                    <tr>
                        <td colspan="6" class="text-end">TOTAL EXPENSE AMOUNT:</td>
