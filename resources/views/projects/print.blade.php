...
<head>
    <meta charset="UTF-8">
    <title>Project Entry Management Report - {{ auth()->user()->firm->name ?? 'ERP Solution' }}</title>
    ...
    <div class="invoice-card">
        <div class="invoice-header-bg d-flex justify-content-between align-items-center">
            <div>
                ...
                <span class="small opacity-75">Project Entry Management Report</span>
            </div>
            <div class="text-end">
                <h4 class="fw-bold mb-0 text-uppercase">Projects Report</h4>
                ...
                <div class="border rounded p-3 bg-light text-center">
                    <div class="row font-outfit fw-bold">
                        ...
                        <div class="col-3 border-end">
                            Total Expenses: ₹{{ number_format($totalExpensesAmount, 2) }}
                        </div>
                        <div class="col-3">
                            Final Amount: ₹{{ number_format($totalFinalAmount, 2) }}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <h6 class="fw-bold text-dark mb-2">Projects List</h6>
            ...
            <tbody>
                @forelse($projects as $index => $project)
                    @php
                        ...
                        $finalVal = $project->po_amount - $expensesTotal;
                    @endphp
                    <tr>
                        <td class="text-center fw-bold">{{ $index + 1 }}</td>
                        <td class="fw-semibold">{{ $project->project_name }}</td>
                        <td>{{ $project->firm->name ?? 'N/A' }}</td>
                        <td class="font-monospace">{{ $project->po_number }}</td>
                        <td class="text-center font-monospace">{{ $project->po_date ? \Carbon\Carbon::parse($project->po_date)->format('d-m-Y') : 'N/A' }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($project->po_amount, 2) }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($expensesTotal, 2) }}</td>
                        <td class="text-end fw-bold font-monospace">₹{{ number_format($finalVal, 2) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-3 text-muted">No projects found.</td>
                    </tr>
                @endforelse
            </tbody>
            @if(count($projects) > 0)
                <tfoot class="table-light border-top font-outfit fw-bold">
                    <tr>
                        ...
                        <td class="text-end font-monospace">₹{{ number_format($totalPoAmount, 2) }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($totalExpensesAmount, 2) }}</td>
                        <td class="text-end font-monospace">₹{{ number_format($totalFinalAmount, 2) }}</td>
                    </tr>
                </tfoot>
            @endif
        </table>
